        ...
        <div v-if="ongoingTournament" 
          ...
          <div class="flex flex-col space-y-4">
            ...
            <div v-if="attendanceStats.has(ongoingTournament.id)" class="mt-4 p-4 bg-gradient-to-r from-blue-50 to-green-50 rounded-lg border border-gray-200">
              <div class="flex justify-between items-center mb-3">
                ...
                <span class="text-sm font-medium text-gray-700 bg-white px-2 py-1 rounded-full">
                  {{ getHighestFieldAttendanceCount(ongoingTournament.id) }} / {{ getAttendanceStats(ongoingTournament.id)?.totalPlayers || 0 }}
                </span>
              </div>
              ...
              <div class="grid gap-2 text-xs" :class="ongoingTournament.selfFunded ? 'grid-cols-1' : 'grid-cols-3'">
                ...
                >
                  <div class="font-semibold text-green-800"><span class="block sm:inline">Sân 5: {{ getAttendanceStats(ongoingTournament.id)?.field5Count || 0 }}</span><span class="block sm:inline"> <span class="hidden sm:inline">/ </span>Sân 7: {{ getAttendanceStats(ongoingTournament.id)?.field7Count || 0 }}</span></div>
            ...
            <div v-if="ongoingTournament.status === 'UPCOMING' && getTournamentTeams(ongoingTournament).length === 0" class="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-4 lg:hidden">
              ...
              <div v-if="getTeamPreviewPlayers(ongoingTournament.id, mobileTeamPreviewField).length" class="space-y-2"><div v-for="player in getTeamPreviewPlayers(ongoingTournament.id, mobileTeamPreviewField)" :key="player.id" class="flex items-center justify-between rounded bg-white p-2 text-sm" :class="{ 'bg-yellow-100': isPlayerBetting(ongoingTournament.id, player.id), 'border-2 border-red-500': isCurrentUserPlayer(player.id) }"><div class="flex min-w-0 items-center"><div class="mr-2 flex h-6 w-6 shrink-0 items-center justify-center overflow-hidden rounded-full bg-gray-300 text-xs font-medium"><img v-if="player.avatar" :src="player.avatar" :alt="player.name" class="h-full w-full object-cover"><span v-else>{{ player.name.charAt(0).toUpperCase() }}</span></div><span class="truncate font-medium text-gray-900">{{ player.name }}<span v-if="player.addedByUsername" class="font-normal text-gray-500"> - thêm bởi {{ player.addedByUsername }}</span></span><span v-if="isPlayerWithWater(ongoingTournament.id, player.id)" class="ml-1">💧</span></div><div class="ml-2 flex shrink-0 items-center text-gray-600"><span class="mr-1 rounded px-1.5 py-0.5 text-xs" :class="isGoalkeeper(player.position) ? 'bg-green-100 font-semibold text-green-700' : ''">{{ getPositionLabel(player.position) }}</span><span class="text-xs">T{{ player.tier }}</span></div></div></div><p v-else class="py-5 text-center text-sm text-gray-500">Chưa có cầu thủ.</p>
            </div>
...
const getTeamPreviewPlayers = (tournamentId: string, field: 'FIELD_5' | 'FIELD_7'): any[] => {
  ...
  const players = details
    .filter((attendance: any) => attendance.status === 'ATTEND' && (field === 'FIELD_5' ? attendance.field5 !== false : attendance.field7 !== false))
    .map((attendance: any) => ({
      ...attendance.player,
